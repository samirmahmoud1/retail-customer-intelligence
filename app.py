...
def get_google_drive_direct_url(file_path: str) -> str:
...
def read_dataframe_from_bytes(file_bytes: bytes, original_path: str = "") -> pd.DataFrame:
    lower_path = original_path.lower().strip()

    # If extension is clear, use it directly
    ...
    raise ValueError(
        "Could not detect file type. Please use a direct CSV/XLSX file path or a valid Google Drive file link."
    )
...
def load_raw_data(file_path: str) -> pd.DataFrame:
    ...
    if "drive.google.com" in file_path:
        download_url = get_google_drive_direct_url(file_path)
        file_bytes = download_bytes(download_url)
        df = read_dataframe_from_bytes(file_bytes, original_path=file_path)
    else:
        ...
        if lower.endswith(".csv"):
            ...
            # Try local file smart detection
...
def prepare_datasets(file_path: str):
    ...
    if df_clean.empty:
        raise ValueError(
            "The cleaned dataset is empty after filtering. "
            "Check that Quantity > 0, UnitPrice > 0, and key columns are present."
        )
...
def train_model(rfm: pd.DataFrame):
    ...
    # Safety in case there is only one class
    if y.nunique() < 2:
        raise ValueError(
            "Churn target has only one class after preprocessing. "
            "The model cannot train on a single-class target."
        )
...
try:
    ...
    st.error("Failed to load the dataset. Check the file path, file type, and required column names.")
    st.exception(e)
    st.stop()
